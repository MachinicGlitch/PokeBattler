...
function BattleArea() {
  ...
  React.useEffect(() => {
    ...
    if (counter === 8) {
      if (numBlueWins === 3) {
        ...
        getPokemon(true);
        getPokemon(false);
      } else if (blueWins) getPokemon(false);
      else if (!blueWins) getPokemon(true);
    }
  ...
  const getPokemon = (isBlue) => {
    axios
      .get(
        "http://localhost:3306/pokemon/" + (Math.floor(Math.random() * 151) + 1)
      )
      .then((res) => {
        const data = res.data;
        ...
        axios.post(
          "http://localhost:3306/battles/updateTimesChosen",
          paramsBattle
        );
        ...
        axios.post(
          "http://localhost:3306/types/updateTimesChosen",
          paramsType1
        );
        ...
        if (isBlue) {
          // set blue pokemon if it is mono type
          ...
          // set blue pokemon if it is dual type
          else
            setBluePokemon({
              id: data.id,
              name: data.name,
          ...
          // set red pokemon if it is mono type
          if (data.types.length === 1)
            setRedPokemon({
              id: data.id,
              name: data.name,
          ...
          // set red pokemon if it is dual type
          else
            setRedPokemon({
              id: data.id,
              name: data.name,
